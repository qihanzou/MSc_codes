...
both_max = matrix(NA, nrow=N_sim_out, ncol=2)
both_min = matrix(NA, nrow=N_sim_out, ncol=2)
llr = matrix(NA, nrow=N_sim_out, ncol=2)
rll = matrix(NA, nrow=N_sim_out, ncol=2)
...
for (t in 1:N_sim_out) {
  ...
  both_max[t,] = theta_sim[which.max(theta_sim0 + theta_sim1),]
  both_min[t,] = theta_sim[which.min(theta_sim0 + theta_sim1),]
  llr[t,] = theta_sim[which.max(theta_sim0 - theta_sim1),]
  rll[t,] = theta_sim[which.max(theta_sim1 - theta_sim0),]
}
...
E_both_max = colMeans(both_max)
E_both_min = colMeans(both_min)
E_llr = colMeans(llr)
E_rll = colMeans(rll)

mm_sim_data = rbind(E_llr, E_rll)#, E_both_max, E_both_min)
...
V2 = cbind(varbeta_rb, mm_Var_Res_sim)
V2_mean = rowMeans(V2)
...
list(alpha_est = out1$beta,
     ...
     varbeta_4m_V2 = as.matrix(V2_mean),
