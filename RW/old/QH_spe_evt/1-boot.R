...
# iters = 100
# boot_alpha = matrix(0, 2, iters)
# boot_theta = matrix(0, 2, iters)
# ETAallb_all = SimuData(out1$theta, rate, locall, cov_xi, nug, iters)
# for (t in 1:iters){
#   x1allb = r1all%*%alpha_est + ETAallb_all$Ymat[t,]
#   x1sb = x1allb[-idx]
#   out_b = MLE.fit(x1sb, r1s, Dall[-idx,-idx], cov_xi, xi.ini, nug, "LB", lo.bound,up.bound)
#   boot_alpha[,t] = out_b$beta
#   boot_theta[,t] = out_b$theta
# }
# 
# boot_a_mean = rowMeans(boot_alpha)
# boot_t_mean = rowMeans(boot_theta)
# 
# alpha0
# alpha_est
# boot_a_mean
# diff_a = alpha_est - boot_a_mean
# a_c = alpha_est + diff_a
# (bias_oa = alpha0 - alpha_est)
# (bias_ca = alpha0 - a_c)
# 
# 
# xi0
# out1$theta
# boot_t_mean
# diff_t = out1$theta - boot_t_mean
# t_c = out1$theta + diff_t
# (bias_ot = xi0 - out1$theta)
# (bias_ct = xi0 - t_c)
# 
# color = c("red", "green","blue")
# A = rbind(as.numeric(xi0), as.numeric(out1$theta), t_c)
# B = data.frame(cbind(A, color))
# 
# plot(x = B$V1, y = B$V2, 
#      col = B$color, pch = 19)
# 
...
for (t in 1:N_sim_out) {
  valid_samples <- rmvnorm(N_sim_in, mean = out1$theta, sigma = sdout$covmat) 
...
p1 = hist(max0v)
p2 = hist(min0v)
plot(p1, xlim=c(min(min0v), max(max0v)))
plot(p2, xlim=c(min(min0v), max(max0v)), add = T)
...
abline(v = mean(max0v) - mean(min0v), col = "red")
...
p3 = hist(max1v)
p4 = hist(min1v)
plot(p3, xlim=c(min(min1v), max(max1v)))
plot(p4, xlim=c(min(min1v), max(max1v)), add = T)
...
abline(v = mean(max1v) - mean(min1v), col = "red")
