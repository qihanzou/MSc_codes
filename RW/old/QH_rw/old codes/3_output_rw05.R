...
resTrf = function(res){
  ...
  for(z in 1:1000){
    ...
    VarRW2 = cbind(VarRW2, res[[z]]$varbeta_rw_max)
    VarRW3 = cbind(VarRW3, res[[z]]$varbeta_rw_mean_cut_minmax)
    VarRW4 = cbind(VarRW4, res[[z]]$varbeta_rw_mean_middle95)
    VarRW5 = cbind(VarRW5, res[[z]]$varbeta_rw_mean_middle90)
    VarRW_v2 = cbind(VarRW_v2, res[[z]]$varbeta_rw_v2_mean)
    VarRW_v3 = cbind(VarRW_v3, res[[z]]$varbeta_rw_v3_mean)
    VarSI = cbind(VarSI, res[[z]]$varbeta_rw_simple)
  ...
  return(list(BetaOLS = BetaOLS, BetaRB = BetaRB, BetaKR = BetaKR, BetaRW = BetaRW,
              VarOLS = VarOLS, VarKR = VarKR, VarRB = VarRB, VarRW1 = VarRW1, VarRW2 = VarRW2,
              VarMC = VarMC, VarRW3 = VarRW3, VarRW4 = VarRW4, VarRW5 = VarRW5,
              VarRW_v2 = VarRW_v2, VarRW_v3 = VarRW_v3, VarSI = VarSI,
              XiRB = XiRB,
              ALPHARB = ALPHARB, 
...
n = 200
nameRdata=strwrap(paste("Qihan_rw05_1000n_", n, "_", n/2, "_mc100_ci99.Rdata", sep=""))
load(nameRdata)
betas0 = c(2,beta1)
out = resTrf(res)
...
TRW_rb_mean = outcover(t(out$BetaRB), t(out$VarRW1)^.5, betas0)
TRW_rb_max = outcover(t(out$BetaRB), t(out$VarRW2)^.5, betas0)
TRW_rb_v2 = outcover(t(out$BetaRB), t(out$VarRW_v2)^.5, betas0)
TRW_rb_v3 = outcover(t(out$BetaRB), t(out$VarRW_v3)^.5, betas0)
TRW_rb_si = outcover(t(out$BetaRB), t(out$VarSI)^.5, betas0)

TRW_rw_mean = outcover(t(out$BetaRW), t(out$VarRW1)^.5, betas0)
TRW_rw_max = outcover(t(out$BetaRW), t(out$VarRW2)^.5, betas0)
TRW_rw_v2 = outcover(t(out$BetaRW), t(out$VarRW_v2)^.5, betas0)
TRW_rw_v3 = outcover(t(out$BetaRW), t(out$VarRW_v3)^.5, betas0)
TRW_rw_si = outcover(t(out$BetaRW), t(out$VarSI)^.5, betas0)
