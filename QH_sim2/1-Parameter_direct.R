...
# --------------------------------------------------------------------------- #
g0_est = beta_RBEGLS[1]  + beta_RBEGLS[2]*alpha_est[1] 
g1_est = beta_RBEGLS[2]*alpha_est[2]
gamma_est = t(cbind(g0_est, g1_est))

residual_est = y - r1%*%gamma_est
cov(residual_est)
mean(diag(CovUBE_GLS))
mean(CovUBE_GLS)
