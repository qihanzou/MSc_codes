...
#run.sim <- function(){
  ...
  # ETAall_all_j = SimuData(out1$theta, rate, locall, cov_xi, nug, dim(samples[outliers,])[1])
  ...
  for (c in 1:dim(samples[outliers,])[1]){
    ...
    ETAall_all_j = SimuData(samples[outliers,][c,], rate, locall, cov_xi, nug, 1)
    x1all_j = r1all%*%alpha_est + ETAall_all_j$Ymat[1,]
    
    #x1all_j = r1all%*%alpha_est + ETAall_all_j$Ymat[c,]
    x1s_j = x1all_j[-idx]
    ...
    alpha_j = solve(t(r1s) %*% tmp_inv_sim %*% r1s)%*%t(r1s) %*% tmp_inv_sim %*% x1s_j 
    
    w_sim = r1%*%alpha_j + cov_xiall_sim[idx, -idx]%*%tmp_inv_sim%*%(x1s_j - r1s%*%alpha_j)
    W_sim = cbind(1, w_sim)
