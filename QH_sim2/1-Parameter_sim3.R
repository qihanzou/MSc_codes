...
num_sim_samples = 200
# # 
R_address = "C:/Users/qihan/Desktop/QH_sim2"
...
theta0_up = out1$theta[1] + 1.96*diag(sdout$covmat)[1]
theta0_down = out1$theta[1] - 1.96*diag(sdout$covmat)[1]
theta1_up = out1$theta[2] + 1.96*diag(sdout$covmat)[2]
theta1_down = out1$theta[2] - 1.96*diag(sdout$covmat)[2]
...
index2 = NULL
for (z in 1:dim(samples[outliers,])[1]){
  xi_sim = samples[outliers,][z,]
  index1 = outliers[z]
  if (xi_sim[1] < xmin || xi_sim[1] > xmax || xi_sim[2] < ymin || xi_sim[2] > ymax) {
    index2 = cbind(index2, index1)
  } 
}


plot(samples[,1], samples[,2])
points(x = samples[outliers,1], y = samples[outliers,2],col = "forestgreen")
points(x = samples[index2,1], y = samples[index2,2],col = "red")
points(x = as.numeric(out1$theta[1]), y = as.numeric(out1$theta[2]), col = "blue", pch = 19)
points(x = as.numeric(set_data[,1]), y = as.numeric(set_data[,2]), col = "brown", pch = 19)
points(x = xi0[1], y = xi0[2], col = "green", pch = 19)
points(x = colMeans(samples[outliers,])[1], 
       y = colMeans(samples[outliers,])[2], col = "red", pch = 19)
segments(x0 = as.numeric(set_data[,1][1]), y0 = as.numeric(set_data[,2][1]), x1 = as.numeric(set_data[,1][2]), y1 = as.numeric(set_data[,2][2]), col = "red", lwd = 3, lty = "dotted")    
segments(x0 = as.numeric(set_data[,1][2]), y0 = as.numeric(set_data[,2][2]), x1 = as.numeric(set_data[,1][4]), y1 = as.numeric(set_data[,2][4]), col = "red", lwd = 3, lty = "dotted")    
segments(x0 = as.numeric(set_data[,1][3]), y0 = as.numeric(set_data[,2][3]), x1 = as.numeric(set_data[,1][1]), y1 = as.numeric(set_data[,2][1]), col = "red", lwd = 3, lty = "dotted")    
segments(x0 = as.numeric(set_data[,1][3]), y0 = as.numeric(set_data[,2][3]), x1 = as.numeric(set_data[,1][4]), y1 = as.numeric(set_data[,2][4]), col = "red", lwd = 3, lty = "dotted")    
...
for (c in 1:dim(samples[index2,])[1]){

  xi_sim = samples[index2,][c,]
...
varbeta_mean = rowMeans(EVarGLS_sim)
...
list(alpha_est = out1$beta,
     ...
     varbeta_mean = as.matrix(varbeta_mean)
)
