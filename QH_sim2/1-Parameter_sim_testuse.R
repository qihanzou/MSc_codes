...
#run.sim <- function(){
  ...
  outliers <- which(F_stat > qf(alpha_level_outliers, df1 = p_samples, df2 = n_samples-p_samples-1, lower.tail=FALSE))
  ...
  plot(EVarGLS_sim[1,], EVarGLS_sim[2,])
  points(colMeans(t(EVarGLS_sim))[1],colMeans(t(EVarGLS_sim))[2], col = "red")
  points(varbeta_rb[1],varbeta_rb[2], col = "blue")
  
 
  
  center = colMeans(t(EVarGLS_sim))
  dist_EVAR = as.numeric(apply(t(EVarGLS_sim),1,function(x,center) {(sqrt((x[1] - center[1])^2+(x[2]-center[2])^2))}, center))
  cbind(t(unname(EVarGLS_sim)), unname(dist_EVAR))
  Nor_dist = dist_EVAR/sum(dist_EVAR)
  #sort(Nor_dist)
  plot(1:length(weight_sample), weight_sample)
  plot(1:length(weight_sample), sort(Nor_dist))
  
  dmean1 = weighted.mean(ss1, sort(Nor_dist))
  dmean2 = weighted.mean(ss2, sort(Nor_dist))
  varbeta_dmean = as.numeric(cbind(dmean1, dmean2))
  #--------------------------------------------------------------------------------------------------------------
  list(alpha_est = out1$beta,
       ...
       varbeta_dmean = as.matrix(varbeta_dmean)
  )
