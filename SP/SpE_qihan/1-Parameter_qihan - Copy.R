...
#run.sim <- function(){
  ...
  for (v in 1:boot_iter){
    ...
    CovUB_i = CovU*beta_i[2]^2 
    CovUBE_GLS_i = CovUB_i + CovE_GLS
    inv_Cov_UBE_GLS_i = solve(CovUBE_GLS_i)
    
    A_i = solve(t(W_i)%*%inv_Cov_UBE_GLS_i%*%W_i)
    varbeta_i = diag(A_i)
  ...
  varbeta_cl0 = diag(A1)
  varbeta_cl1 = 1/boot_iter*colSums(var_boot)
