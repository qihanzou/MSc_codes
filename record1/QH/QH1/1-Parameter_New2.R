...
run.sim <- function(){
  ...
  max(distall)
  min(distall)
  i_max = which(distall == max(distall))
  i_min = which(distall == min(distall))
  plot(locall$x,locall$y)
  points(0,0, col = "blue", pch = 19)
  points(locall$x[i_max],locall$y[i_max], col = "red", pch = 19)
  points(locall$x[i_min],locall$y[i_min], col = "red", pch = 19)
  # ---------------------------------------------------------------------------#
  
  
  # predictors, -idx for "s"
  ...
  for (v in 1:boot_iter){
    ...
    #xie = out1$theta
    cov_xialle = cor.mat(Dall, xie[1], cov_xi, nug = 0) # ? sigma^2?
    
    # (c): plug parameters into (2.4) kriging equation to derive Wj:
    tmp_inv = solve(cov_xialle[-idx,-idx])
    alphaboot = solve(t(r1s) %*% tmp_inv %*% r1s)%*%t(r1s) %*% tmp_inv %*% x1sboot # I change this for trial, previously x1s
