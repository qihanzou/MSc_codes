## Calculate the covarince matrix (cov.gau) and standard error of each estiamtor (s.gau)
SDCal <- function (D,theta, cov.model){
  
  if (cov.model != "PExp"){
    Jout = Jmat(theta, cov.model=cov.model, D = D)
...
Jmat = function (theta, cov.model, D){
