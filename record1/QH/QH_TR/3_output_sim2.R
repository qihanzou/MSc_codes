...
resTrf = function(res){
  ...
  for(z in 1:1000){
    ...
    VarSI    = cbind(VarSI, res[[z]]$varbeta_gls_mean ) #varbeta_wmean)
...
setwd("C:/Users/qihan/Desktop/QH_sim2")

nameRdata=strwrap(paste("sim_p1_1000_n_200_100_s1000.Rdata"))
load(nameRdata)
betas0 = c(2,beta1)
out = resTrf(res)
...
TSI = outcover(t(out$BetaRB), t(out$VarSI)^.5, betas0)
...
library("xtable")  
T1 = cbind(t(TO), t(TKR), t(TRB), t(TMC), t(TSI))
