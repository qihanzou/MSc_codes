...
load("1-Data_N1300_O3N2_S2DIG_rs1000.Rdata")
...
D_gas = deprojected_distances2(Local_gas$RA, Local_gas$DEC, Local_gas$RA, Local_gas$DEC, 278, 61.1842)
D_data = deprojected_distances2(Local_data$RA, Local_data$DEC, Local_data$RA, Local_data$DEC, 278, 61.1842)
D_data_to_gas = deprojected_distances2(Local_data$RA, Local_data$DEC, Local_gas$RA, Local_gas$DEC, 278, 61.1842)
x1s = as.matrix(select(Local_gas, c("CO21_mgsd")))
...
y.comb = as.matrix(new_data$Z_O3N2) # change!!!

cv_k = 5
n.comb = length(y.comb)
if (n.comb%%cv_k == 0){
  split.ind = rep(1:cv_k, each = floor(n.comb/cv_k)) # a bug here previously, not affect previous results.
}else{
...
for(j in 1:cv_k){
  ...
  kr = MLE.fit(Ytrain, Wtrain, D_data[idx_train,idx_train], cov_ep, theta.ini, nug = 0, "LB", lo.bound, up.bound)
  theta_kr = kr$theta
  beta_kr  = kr$beta
  Sigma_kr = theta_kr[2]*exp(-D_data[idx_train,idx_train]/theta_kr[1]) 
  cmat_kr = theta_kr[2]*exp(-D_data[idx_test,idx_train]/theta_kr[1]) 
  pl_kr = as.matrix(Wtest)%*%beta_kr
  ...
  RBE_res = RBEGLS_loop2(50, 0.001, 0.001, beta_ols, theta_ep.ini, 
                         CovU[idx_train, idx_train], Ytrain, Wtrain, D_data[idx_train,idx_train], 
  ...
  pl_RBE = as.matrix(Wtest)%*%beta_RBEGLS
  pe_RBE = cc1%*% solve(CovUBE_GLS)%*%(Ytrain - Wtrain%*%beta_RBEGLS) 
