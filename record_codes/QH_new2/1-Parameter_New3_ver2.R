...
R_address = "C:/Users/qihan/Desktop/QH_new2"
...
#run.sim <- function(){
  ...
  x3s = as.matrix(x3all[-idx]) # originally, I have
  # -------------------------------------------------- #
  # case 1:
  ...
  x4s = as.matrix(x4all[-idx]) # originally, I have
  # -------------------------------------------------- #
  # case 2: 
  ...
  x2 = distall[idx]
  y = beta0 + beta1*x1 + beta2*x2 + beta3*x3 + beta4*x4 + ep_error     # create y values for s1,...,sN
  ...
  for (v in 1:boot_iter){
    ...
    # case 2: for x4
    # -------------------------------------------------- #
    x4allboot = p4all%*%zeta_est + ETAallboot_all_w4$Ymat[v,]
    ...
    alphaboot = solve(t(r1s) %*% tmp_inv %*% r1s)%*%t(r1s) %*% tmp_inv %*% x1sboot # why we use x1s rather than x1sboot here?
    wboot_e = r1%*%alphaboot + cov_xialle[idx, -idx]%*%tmp_inv%*%(x1sboot - r1s%*%alphaboot)
    
    tmp_inv_w3 = solve(cov_kappaalle[-idx_new,-idx_new])
    gammaboot = solve(t(p3obs) %*% tmp_inv_w3 %*% p3obs)%*%t(p3obs) %*% tmp_inv_w3 %*% x3obsboot
    ...
    zetaboot = solve(t(p4obs) %*% tmp_inv_w4 %*% p4obs)%*%t(p4obs) %*% tmp_inv_w4 %*% x4obsboot
    ...
    # ------------------------------------------------------------------------- #
    # ------------------------------------------------------------------------- #
    fols_j = lm(y~Wboot_e-1)
    beta_ols_j = fols_j$coef
    
    
    pc_j = r1 - cov_xialle[idx, -idx]%*%solve(cov_xialle[-idx,-idx]) %*% r1s
    CovU_j = cov_xialle[idx, idx] - cov_xialle[idx, -idx]%*%solve(cov_xialle[-idx,-idx]) %*% t(cov_xialle[idx, -idx]) + (pc_j)%*%solve(t(r1s)%*%solve(cov_xialle[-idx,-idx])%*%r1s)%*%t(pc_j)
    CovUB_w1_j = CovU_j*beta_ols_j[2]^2 
    
    pc_w3_j = p3tar - cov_kappaalle[idx_new, -idx_new]%*%solve(cov_kappaalle[-idx_new,-idx_new]) %*% p3obs
    CovU_w3_all_j = cov_kappaalle[idx_new, idx_new] - cov_kappaalle[idx_new, -idx_new]%*%solve(cov_kappaalle[-idx_new,-idx_new]) %*% t(cov_kappaalle[idx_new, -idx_new]) + (pc_w3_j)%*%solve(t(p3obs)%*%solve(cov_kappaalle[-idx_new,-idx_new])%*%p3obs)%*%t(pc_w3_j)
    CovU_w3_j = CovU_w3_all_j[idx,idx]
    CovUB_w3_j = CovU_w3_j*beta_ols[4]^2
    
    pc_w4_j = p4tar - cov_iotaalle[idx_new4, -idx_new4]%*%solve(cov_iotaalle[-idx_new4,-idx_new4]) %*% p4obs
    CovU_w4_partial_j = cov_iotaalle[idx_new4, idx_new4] - cov_iotaalle[idx_new4, -idx_new4]%*%solve(cov_iotaalle[-idx_new4,-idx_new4]) %*% t(cov_iotaalle[idx_new4, -idx_new4]) + (pc_w4_j)%*%solve(t(p4obs)%*%solve(cov_iotaalle[-idx_new4,-idx_new4])%*%p4obs)%*%t(pc_w4_j)
    zero_matrix_j = CovU_w4_partial_j*0
    mm1_j = cbind(CovU_w4_partial_j, zero_matrix_j)
    mm2_j = cbind(zero_matrix_j, zero_matrix_j)
    CovU_w4_j = rbind(mm1_j,mm2_j)
    CovUB_w4_j = CovU_w4_j*beta_ols[5]^2
    
    
    
    CovUB_j = CovUB_w1_j + CovUB_w3_j + CovUB_w4_j
    CovUBE_GLS_j = CovUB_j + CovE_GLS
    CovUBE_GLS_inv_j = solve(CovUBE_GLS_j)
    beta_j = solve(t(Wboot_e)%*%CovUBE_GLS_inv_j%*%Wboot_e)%*%t(Wboot_e)%*%CovUBE_GLS_inv_j%*%y
