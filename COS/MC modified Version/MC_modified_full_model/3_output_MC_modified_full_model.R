...
resTrf = function(res){
  BetaOLS  = BetaKR = BetaRB = XiE = ThetaE = ThetaKR = ALPHA = ZETA = IOTA = NULL
  ...
  for(i in 1:nreps){
    ...
    ThetaKR = rbind(ThetaKR, res[[i]]$theta_kr)
    
    ALPHA = rbind(ALPHA, t(res[[i]]$alpha_est))
    ZETA = rbind(ZETA, res[[i]]$zeta_est)
    IOTA = rbind(IOTA, res[[i]]$iota_est)
  }
  return(list(BetaOLS = BetaOLS,BetaKR = t(BetaKR), BetaRB = t(BetaRB), 
              ...
              VarKRT = VarKRT, VarKRTm = VarKRTm, XiE = XiE, ThetaE = ThetaE, ThetaKR = ThetaKR,
              ALPHA = ALPHA, ZETA = ZETA, IOTA = IOTA))
}
...
setwd("C:/Users/qihan/Desktop/MC_modified_full_model")
#setwd("C:/Users/qihan/Desktop/QH_new/res1")

  n = 200
  nameRdata=strwrap(paste("MCM_beta1is4_zeta0is2_X1000n_200_100.Rdata"))
  load(nameRdata)
  betas0 = c(2,beta1, 1, 3)
  out = resTrf(res)
  ...
  Tab0 = TabK = TabKm = TabR = TabRm = TabKT = TabKTm  = matrix(0,12,6)
  TabXi= TabThe = matrix(0,12,3)
  for (fi in 1:4){
    nsam = c(200, 400, 800, 1600)
    n = nsam[fi]
    nameRdata=strwrap(paste("MCM_beta1is1_zeta0is2_X1000n_", n, "_", n/2, ".Rdata", sep=""))
    load(nameRdata)
    ...
    Tab0[1:3+(fi-1)*3, ] = T0[-1,]
    TabK[1:3+(fi-1)*3, ] = TK[-1,]
    TabKm[1:3+(fi-1)*3, ] = TK2[-1,]
    TabR[1:3+(fi-1)*3, ] = TR1[-1,]
    TabRm[1:3+(fi-1)*3, ] = TR2[-1,]
    TabKT[1:3+(fi-1)*3, ] = TKT[-1,]
    TabKTm[1:3+(fi-1)*3, ] = TKT2[-1,]
