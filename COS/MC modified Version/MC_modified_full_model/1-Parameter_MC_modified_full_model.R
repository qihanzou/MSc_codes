...
source("RBEGLS_functions_New4.R")
...
run.sim <- function(){
  ...
  etaall4 = SimuData(iota0, rate, locall, cov_iota, nug, 1) 
  ...
  M = length(x1s)
  # -------------------------------------------------------------------------- #
  p4all = as.matrix(cbind(rep(1, nall))) 
  p4s = p4all[-idx,]
  p4 = p4all[idx]
  ...
  p4tar = p4all[idx_new4,]
  p4obs = p4all[-idx_new4,]
  # -------------------------------------------------------------------------- #
  
  #  simple model
  ep_error = epall$Ymat[k,idx]        # error of s1,...,sN
  x2 = distall[idx]
  y = beta0 + beta1*x1 + beta2*x2 + beta4*x4 + ep_error     # create y values for s1,...,sN
  
  
  # -------------------------------------------------- #
  # 1. compute w1, w4 and OLS beta estimates
  # -------------------------------------------------- #
  # for w1.
  # -------------------------------------------------- #
  ...
  CovUB_w4 = CovU_w4*beta_ols[4]^2
  # -------------------------------------------------- #

  CovUB = CovUB_w1 + CovUB_w4
  ...
  # loop start here:
  # -------------------------------------------------- #
  res1 = RBEGLS_loop_New4(50, 0.001, 0.001, beta_RBEGLS, out2$theta, theta_ep.ini, CovU, CovU_w4, y, W, Dall, cov_ep, lo.bounde,up.bounde)
  ...
  CovUB = CovU*beta_RBEGLS[2]^2 + CovU_w4*beta_RBEGLS[4]^2
  CovUBE_GLS = CovUB + CovE_GLS
  ...
  EVar = EVarKR = EVarKRT = matrix(0, 4, 4)
  # -------------------------------------------------- #
  # step3: start here:
  for (v in 1:boot_iter){
    ...
    while ((iotae[1] < 0)|(iotae[2] < 0)) {
      iotae = out_w4$theta + rmvnorm(1, sigma = sdout_w4$covmat)
    }
    cov_iotaalle = iotae[2]*cor.mat(Dall, iotae[1], cov_iota, nug = 0)
    
    
    # (b):
    ETAallboot_all = SimuData(xie, rate, locall, cov_xi, nug, 1)
    ETAallboot_all_w4 = SimuData(iotae, rate, locall, cov_iota, nug, 1)
    
    x1allboot = r1all%*%alpha_est + ETAallboot_all$Ymat[1,]
    x1sboot = x1allboot[-idx]
    
    x4allboot = p4all%*%zeta_est + ETAallboot_all_w4$Ymat[1,]
    ...
    w4boote = as.matrix(c(wboot4_partial, x4obsboot[1:(length(idx)/2)]))
    
    
    Wboot_e = cbind(1, wboot_e, x2, w4boote)
    # ------------------------------------------------------------------------- #
