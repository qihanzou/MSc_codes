...
Wilks.function <- function(dat){
  ...
  p <- 1 - pf(F.stat, p, n-p-1) # p value for each row
  cbind(w, F.stat, p)
}


dat.rows <- Wilks.function(samples)
...
EVar_sim  = EVarGLS_sim = matrix(0, 2, 2)
for (c in 1:dim(samples[outliers,])[1]){
  ...
  EVarGLS_sim = EVarGLS_sim + varbeta_sim_gls
  
  
}
varbeta_sim2 = diag(EVar_sim/dim(samples[outliers,])[1])
varbeta_sim_gls = diag(EVarGLS_sim/dim(samples[outliers,])[1])
...
list(alpha_est = out1$beta,
     ...
     varbeta_sim = as.matrix(varbeta_sim_gls)
)
