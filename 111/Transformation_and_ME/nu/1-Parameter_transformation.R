...
eet_set = NULL
eet = matrix(0, length(idx), length(idx))
for (k in 1:N_sim){
  Zk = cbind(1, Z_set[k,])
  folsk = lm(y~Zk-1)
  beta_olsk = folsk$coef
  
  ek = y - Zk%*%beta_olsk
  eet_set = cbind(eet_set, ek)
}
E_centered <- scale(eet_set, center = TRUE, scale = FALSE)  # center only
Sigma_hat <- E_centered %*% t(E_centered) / (ncol(eet_set) - 1)
qr(Sigma_hat)$rank
...
bures_distance <- function(A, B) {
  # epsilon <- 1e-6
  A <- (A + t(A)) / 2 + epsilon * diag(nrow(A))
  B <- (B + t(B)) / 2 + epsilon * diag(nrow(B))
  # print(eigen(A)$values)
  # print(eigen(B)$values)
  A_half <- sqrtm(A)
...
fit_bures <- function(theta, D, Rm, KSigma) {
  sigma2 <- theta[1]
  phi <- theta[2]
  
  S_model <- sigma2 * exp(-D / phi) 
  S_model <- S_model + KSigma
 
  dist2 <- bures_distance(Rm, S_model)
  return(dist2)
}
...
fit <- optim(
  ...
  fn = fit_bures,
  D = Dall[idx,idx],
  Rm = CovUBE_GLS, #Sigma_hat,
  KSigma = CovU_nl*beta_RBEGLS[2]^2,
  method = "Nelder-Mead",
  lower = c(1e-6, 1e-3),
  upper = c(20, 20)
)
