...
for (v in 1:MC_iter){
  ...
  gdist_e = DT_Delta_1st_jacobianfun(wboot_e, CovU_e, g) # 1st order Delta method
...
for (v2 in 1:MC_iter){
  ...
  alphaboot2 = solve(t(r1s) %*% tmp_inv2 %*% r1s)%*%t(r1s) %*% tmp_inv2 %*% x1s
  wboot_e2 = r1%*%alphaboot2 + cov_xialle2[idx, -idx]%*%tmp_inv2%*%(x1s - r1s%*%alphaboot2)
  ...
  gdist_e2 = DT_Delta_1st_jacobianfun(wboot_e2, CovU_e2, g) # 1st order Delta method
