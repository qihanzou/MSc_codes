...
gdist = DT_Delta_1st(w, CovU, g, dg) # 1st order Delta method
CovU_nl = gdist_mc$cov_z
z = gdist_mc$z
...
EVar = EVar_2  = EVar2  = EVar2_2 = matrix(0, 2, 2)
for (v in 1:MC_iter){
  ...
  gdist_e = DT_MC(wboot_e, CovU_e, g, 100)
  #gdist_e = DT_Delta_1st(wboot_e, CovU_e, g, dg) # 1st order Delta method
  CovU_nl_e = gdist_e$cov_z
  Z_e = cbind(1, gdist_e$z)
  CovUB_e = CovU_nl_e*beta_RBEGLS[2]^2
  CovUBE_GLS_e = CovUB_e + CovE_GLS #+ diag(1e-3, nrow(CovE_GLS))
  ...
  ETAallboot_all2 = SimuData(xie, rate, locall, cov_xi, nug, 1)
  x1allboot2 = r1all%*%alpha_est + ETAallboot_all2$Ymat[1,]
  x1sboot2 = x1allboot2[-idx]
  cov_xialle2 = xie[2]*cor.mat(Dall, xie[1], cov_xi, nug = 0)
  ...
  gdist_e2 = DT_MC(wboot_e2, CovU_e2, g, 100)
  #gdist_e2 = DT_Delta_1st(wboot_e2, CovU_e2, g, dg) # 1st order Delta method
  CovU_nl_e2 = gdist_e2$cov_z
  Z_e2 = cbind(1, gdist_e2$z)
  CovUB_e2 = CovU_nl_e2*beta_RBEGLS[2]^2
  CovUBE_GLS_e2 = CovUB_e2 + CovE_GLS #+ diag(1e-3, nrow(CovE_GLS))
