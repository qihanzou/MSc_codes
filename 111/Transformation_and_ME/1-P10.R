...
EV_dm = EV_mc = NULL
MEVar_dm = MEVar_mc = matrix(0, 2, 2)
MEV_dm = MEV_mc = NULL
...
for (v in 1:MC_iter){
  ...
  pce = r1 - cov_xialle[idx, -idx]%*%solve(cov_xialle[-idx,-idx]) %*% r1s
  CovUe = cov_xialle[idx, idx] - cov_xialle[idx, -idx]%*%solve(cov_xialle[-idx,-idx]) %*% t(cov_xialle[idx, -idx]) + (pce)%*%solve(t(r1s)%*%solve(cov_xialle[-idx,-idx])%*%r1s)%*%t(pce)
  gdiste = DT_Delta_1st_jacobianfun(wboot_e, CovUe, g) # 1st order Delta method
  CovU_nle = gdiste$cov_z
  ze = gdiste$z
  CovUBe = CovU_nle*beta_RBEGLS[2]^2
  
  CovUBE_GLSe_dm = CovUBe + CovE_GLS
  CovUBE_GLS_inve_dm = solve(CovUBE_GLSe_dm)
  CovUBE_GLSe_mc = CovUBe + CovE_GLS_mc
  CovUBE_GLS_inve_mc = solve(CovUBE_GLSe_mc)
  
  Z_e = cbind(1, ze)
  C1_e = solve(t(Z_e)%*%CovUBE_GLS_inve_dm%*%Z_e)
  D1_e = solve(t(Z_e)%*%CovUBE_GLS_inve_mc%*%Z_e)
  
  
  MEVar_dm = MEVar_dm + C1_e   
  MEV_dm = cbind(MEV_dm, diag(C1_e))
  MEVar_mc = MEVar_mc + D1_e   
  MEV_mc = cbind(MEV_mc, diag(D1_e))
...
varbeta_mc_dm_M = diag(MEVar_dm/MC_iter)
median_pair_dm_M <- c(median(MEV_dm[1, ]), median(MEV_dm[2, ]))
varbeta_mc_mc_M = diag(MEVar_mc/MC_iter)
median_pair_mc_M <- c(median(MEV_mc[1, ]), median(MEV_mc[2, ]))
...
EV_dm2 = EV_mc2 = NULL
MEVar_dm2 = MEVar_mc2 = matrix(0, 2, 2)
MEV_dm2 = MEV_mc2 = NULL
...
for (v2 in 1:MC_iter){
  ...
  gdiste2 = DT_Delta_1st_jacobianfun(wboot_e2, CovUe2, g) # 1st order Delta method
  CovU_nle2 = gdiste2$cov_z
  ze2 = gdiste2$z
  CovUBe2 = CovU_nle2*beta_RBEGLS[2]^2
  
  CovUBE_GLSe_dm2 = CovUBe2 + CovE_GLS
  CovUBE_GLS_inve_dm2 = solve(CovUBE_GLSe_dm2)
  CovUBE_GLSe_mc2 = CovUBe2 + CovE_GLS_mc
  CovUBE_GLS_inve_mc2 = solve(CovUBE_GLSe_mc2)
  
  Z_e2 = cbind(1, ze2)
  C1_e2 = solve(t(Z_e2)%*%CovUBE_GLS_inve_dm2%*%Z_e2)
  D1_e2 = solve(t(Z_e2)%*%CovUBE_GLS_inve_mc2%*%Z_e2)
  
  
  MEVar_dm2 = MEVar_dm2 + C1_e2   
  MEV_dm2 = cbind(MEV_dm2, diag(C1_e2))
  MEVar_mc2 = MEVar_mc2 + D1_e2   
  MEV_mc2 = cbind(MEV_mc2, diag(D1_e2))
...
varbeta_mc_dm2_M = diag(MEVar_dm2/MC_iter)
median_pair_dm2_M <- c(median(MEV_dm2[1, ]), median(MEV_dm2[2, ]))
varbeta_mc_mc2_M = diag(MEVar_mc2/MC_iter)
median_pair_mc2_M <- c(median(MEV_mc2[1, ]), median(MEV_mc2[2, ]))
...
list(alpha_est = as.matrix(out1$beta),
     ...
     varbeta_mc_mean_dm_M = as.matrix(as.numeric(varbeta_mc_dm_M)),
     varbeta_mc_median_dm_M = as.matrix(as.numeric(median_pair_dm_M)),
     varbeta_mc_mean_mc_M  = as.matrix(as.numeric(varbeta_mc_mc_M)),
     varbeta_mc_median_mc_M = as.matrix(as.numeric(median_pair_mc_M)),
     
     varbeta_mc_mean_dm2_M = as.matrix(as.numeric(varbeta_mc_dm2_M)),
     varbeta_mc_median_dm2_M = as.matrix(as.numeric(median_pair_dm2_M)),
     varbeta_mc_mean_mc2_M  = as.matrix(as.numeric(varbeta_mc_mc2_M)),
     varbeta_mc_median_mc2_M = as.matrix(as.numeric(median_pair_mc2_M))
