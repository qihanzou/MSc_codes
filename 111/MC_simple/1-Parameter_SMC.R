...
lambda1 <- lambda[1] 
lambda2 <- lambda[2] 
vec <- eigen_result$vectors
v1 <- vec[, 1] 
v2 <- vec[, 2]

chi_sq_critical_value <- qchisq(0.95, df = df)
semi_major_axis_length <- sqrt(lambda1 * chi_sq_critical_value)
semi_minor_axis_length <- sqrt(lambda2 * chi_sq_critical_value)

vertex1 <- mu + semi_major_axis_length * v1
vertex2 <- mu - semi_major_axis_length * v1
covextex1 <- mu + semi_minor_axis_length * v2
covextex2 <- mu - semi_minor_axis_length * v2

vertexdata = rbind(vertex1, vertex2, covextex1, covextex2)
