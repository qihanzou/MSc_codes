...
trainset = pd.read_pickle('C:/Users/qihan/Desktop/q/MUSE_pkl/N1385_ver1_copt.pkl') 
arm_musk = pd.read_pickle("C:/Users/qihan/Desktop/q/MUSE_pkl/N1385_ver1_copt_Arm_Musk.pkl")
gal_name = 'N1385'
...
X4 = np.transpose(X4)
plt.scatter(X4[:, 0], X4[:, 1], c = data1['I_arm'], marker='.', s=0.5)
plt.colorbar()
...
RA1 = trainset['RA']
DEC1 = trainset['DEC']
#deprojected_distances_MUSE(Distance, PA_galaxy, inc_angle, RA1, DEC1, RA2 = RA1[0:1000], DEC2 = DEC1[0:1000])


sp_RA = np.array_split(RA1, 20)
sp_DEC = np.array_split(DEC1, 20)


dist_full = list()
for i in range(20):
    print(i)
    dist = deprojected_distances_MUSE(Distance, PA_galaxy, inc_angle, RA1, DEC1, RA2 = sp_RA[i], DEC2 = sp_DEC[i])
    #np.concatenate((A, B))
    dist_full.append(dist)



distm1 = np.concatenate((dist_full[0], dist_full[1], dist_full[2], dist_full[3], dist_full[4]), axis=1)
distm2 = np.concatenate((dist_full[5], dist_full[6], dist_full[7], dist_full[8], dist_full[9]))
distm3 = np.concatenate((dist_full[10], dist_full[11], dist_full[12], dist_full[13], dist_full[14]))
distm4 = np.concatenate((dist_full[15], dist_full[16], dist_full[17], dist_full[18], dist_full[19]))


dist_all = np.concatenate((distm1, distm2, distm3, distm4))
'''
