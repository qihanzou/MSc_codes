# -*- coding: utf-8 -*-
"""
Created on Thu Aug 22 07:21:26 2024
...
Yf_list = list()
for train_index, test_index in kf.split(X1, Y1):
    X_train1 = trainset["proj_dist"][train_index]
    X_train2 = trainset["SFR_density"][train_index]
    X_train3 = trainset["S2_DIG"][train_index]
    Y_train = trainset["Z_N2S2Ha"][train_index]
    
    X_test1 = trainset["proj_dist"][test_index]
    X_test2 = trainset["SFR_density"][test_index]
    X_test3 = trainset["S2_DIG"][test_index]
    Y_test = trainset["Z_N2S2Ha"][test_index]
    ...
    X_test = pd.concat([X_test1, X_test2, X_test3], axis=1)
...
plt.title('O3N2 Geo-model with Distance + SFR + S2DIG (HII)') # change name here. + Kumari 19 correction
plt.xlabel('log([O/H])+12 measured')
plt.ylabel('log([O/H])+12 predicted')

plt.savefig('pic_for_Ben_N2S2Ha_prediction_for_HII_S2DIG_O3N2_2.png', dpi=300)
'''
