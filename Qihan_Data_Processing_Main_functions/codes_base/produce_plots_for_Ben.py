# -*- coding: utf-8 -*-
"""
Created on Tue Aug 20 14:44:02 2024
...
plt.hist2d(Y_true.values, Y_pred.values, 
           bins = 100,  
           norm = colors.LogNorm(),  
           cmap = 'plasma', range = [[8.2, 9.6], [8.2, 9.6]]) # plt.cm.BuPu, plt.cm.jet
           #cmap =plt.cm.jet, range = [[8.5, 8.9], [8.5, 8.9]])
plt.title('N2S2Ha Geo-model with Distance + SFR + S2DIG (DIG)') # change name here. + Kumari 19 correction
plt.xlabel('log([O/H])+12 measured')
plt.ylabel('log([O/H])+12 predicted')
#plt.savefig('pic_for_Ben_N2S2Ha_prediction_for_DIG_2.png', dpi=300)
...
index_above_0_Z = data_galaxy["Z_N2S2Ha"] > 0 # ignore NA Z_N2S2Ha Z_RS32 Z_O3N2
...
plt.scatter(X[:, 0], X[:, 1], c = log(testset[["SFR_density"]].values), marker='.', s=1, cmap = 'plasma')
ax = plt.gca()
ax.set_xlim([-10, 10])
ax.set_ylim([-10, 10])
plt.title("SFR density (with log scale)")
plt.colorbar()
plt.xlabel("x (kpc)")
plt.ylabel("y (kpc)")
plt.grid()
...
plt.scatter(X[:, 0], X[:, 1], c = testset[["S2_DIG"]].values, marker='.', s=1, cmap = 'plasma')
ax = plt.gca()
ax.set_xlim([-10, 10])
ax.set_ylim([-10, 10])
plt.title("S2_DIG")
...
plt.scatter(X[:, 0], X[:, 1], c = testset[["proj_dist"]].values, marker='.', s=1, cmap = 'plasma')
ax = plt.gca()
ax.set_xlim([-10, 10])
ax.set_ylim([-10, 10])
plt.title("Distance")
...
plt.scatter(X[:, 0], X[:, 1], c = Y_pred, marker='.', s=1, cmap = 'plasma')
ax = plt.gca()
ax.set_xlim([-10, 10])
ax.set_ylim([-10, 10])
plt.title("Prediction for DIG regions (N2S2Ha)")
plt.colorbar()
plt.xlabel("x (kpc)")
plt.ylabel("y (kpc)")
plt.grid()
...
plt.scatter(X[:, 0], X[:, 1], c = Y_true - Y_pred, marker='.', s=1, cmap = 'plasma')
ax = plt.gca()
ax.set_xlim([-10, 10])
ax.set_ylim([-10, 10])
plt.title("Residual")
